...
/** Builds the Hardhat sample project for a contract, e.g. `zipHardhat` or `zipHardhatViem`. */
type ZipHardhat = (c: Contract, opts?: GenericOptions) => Promise<JSZip>;

/**
 * Registers the shared Hardhat download test suite against a specific zip generator.
 *
 * Each caller is its own AVA test file, so snapshots stay separate per generator while the
 * fixture matrix and the extract-install-run plumbing are defined once.
 */
export function registerHardhatZipTests(zipHardhat: ZipHardhat) {
